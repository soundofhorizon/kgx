...
class AdminOnly(commands.Cog):
    ...
    async def _get(self, ctx, user: discord.Member):
        cur.execute("SELECT level FROM caution WHERE user_id = %?", int(user.id))
        data = cur.fetchone()
        if len(data) == 0:
            caution_level = 0
        else:
    ...
    async def set(self, ctx, user: discord.Member, n: int):
        cur.execute("DELETE FROM caution WHERE user_id = %?", int(user.id))
        if n == 0:
            db.commit()
            return await ctx.send(f'{user}の警告レベルをリセットしました')
        cur.execute("INSERT INTO caution values (%?, %?)", (int(user.id), n))
        db.commit()
